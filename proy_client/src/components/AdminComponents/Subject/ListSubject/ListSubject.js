...
import CreateSubject from "../AddSubject";
...
export default function ListSubject({ children }) {
  ...
  const readSubject = (code) => {
    setContentModal({
      content: <ReadSubject>{{ subjects: subjects, code: code }}</ReadSubject>,
  ...
  const createSubject = () => {
    setContentModal({
      content: (
        <div>
          <CreateSubject>
            {{ setModal: setModal, getRenderSubjecs: getRenderSubjecs }}
          </CreateSubject>
        </div>
      ),
    ...
    <div>
      ...
      <div className="headerListSubject">
        ...
        <div className="headerListSubject__filterPia">
          <Button
            className="headerListSubject__filterPia__label"
            type="primary"
            onClick={filterPiaaVersion}
          >
            Filtrar por PIAA
          </Button>
          <input
            className="headerListSubject__filterPia__input"
            onChange={(e) => setPiaFilter(e.target.value)}
            defaultValue={1}
            type="number"
            min={1}
          />
          ...
          <List.Item>
            <div className="subjects__conntent">
              ...
              <div className="subjects__conntent__buttons">
                ...
                <Button
                  type="primary"
                  onClick={() => readSubject(subject.activity_code)}
                >
                  <FullscreenOutlined />
                </Button>
                {/* botón de editar */}
                <Button
                  name={subject.activity_code}
                  type="primary"
                  onClick={() => null}
                >
                  <EditOutlined />
                ...
                <Button
                  ...
                  type="danger"
                  onClick={() =>
                    deleteOneSubject(subject._id, subject.academic_activity)
                  }
                >
                  <DeleteFilled />
                </Button>
